...
export default function AdminView({
  ...
  // UI state
  const [activeTab, setActiveTab] = useState<'orders' | 'users' | 'inventory' | 'spaces' | 'capacity' | 'standards'>('orders');
  ...
  useEffect(() => {
    loadRegisteredUsers();
    const handleUpdate = () => loadRegisteredUsers();
    window.addEventListener('registeredUsersUpdated', handleUpdate);
    return () => {
      window.removeEventListener('registeredUsersUpdated', handleUpdate);
    };
  }, []);

  const togglePasswordShow = (phone: string) => {
    setShowPasswordMap(prev => ({ ...prev, [phone]: !prev[phone] }));
  };

  const handleDeleteUser = (phone: string, name: string) => {
    if (window.confirm(`Are you sure you want to delete customer account for ${name} (${phone})?`)) {
      const updated = registeredUsers.filter(u => u.contactNumber !== phone);
      setRegisteredUsers(updated);
      localStorage.setItem('registeredUsers', JSON.stringify(updated));
      try {
        const active = JSON.parse(localStorage.getItem('activeUser') || 'null');
        if (active && active.contactNumber === phone) {
  ...
  const filteredUsers = useMemo(() => {
    if (!userSearchQuery.trim()) return registeredUsers;
    const query = userSearchQuery.toLowerCase().trim();
    return registeredUsers.filter(
      u =>
        u.fullName.toLowerCase().includes(query) ||
        u.contactNumber.includes(query) ||
    ...
    <div className="space-y-8 pb-24" id="admin-workspace-container">
      ...
      <div className="flex border-b border-slate-200 gap-6 overflow-x-auto scrollbar-none">
        ...
        <button
          ...
          className={`pb-3 text-sm font-bold border-b-2 transition uppercase tracking-wider whitespace-nowrap flex items-center gap-1.5 ${activeTab === 'users' ? 'border-amber-600 text-amber-700' : 'border-transparent text-slate-500 hover:text-slate-900'}`}
        >
        ...
        <div className="space-y-6 animate-fadeIn" id="admin-users-tab">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
              ...
              <div>
                ...
                <span className="text-2xl font-black text-slate-900">{registeredUsers.length}</span>
              </div>
            </div>
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
              ...
              <div>
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Active Ordering Customers</span>
                <span className="text-2xl font-black text-slate-900">
                  {registeredUsers.filter(u => orders.some(o => o.details.customerPhone === u.contactNumber)).length}
                </span>
              </div>
            </div>
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
              ...
              <div>
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Total User Orders</span>
                <span className="text-2xl font-black text-slate-900">
                  {orders.filter(o => registeredUsers.some(u => u.contactNumber === o.details.customerPhone)).length}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
            <div className="relative flex-1 max-w-md">
              ...
              <input
                type="text"
                placeholder="Search user by name, phone, address, or PIN..."
            ...
            <button
              ...
              className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl transition-colors flex items-center justify-center gap-2 cursor-pointer"
            >
          ...
          {filteredUsers.length === 0 ? (
            <div className="bg-white rounded-3xl border border-slate-100 p-12 text-center space-y-3 shadow-sm">
              ...
              <h4 className="text-base font-bold text-slate-900">No Registered Users Found</h4>
              <p className="text-xs text-slate-400 max-w-md mx-auto">
                {userSearchQuery 
                  ? 'No registered customers match your search criteria.' 
                  : 'When customers sign up or register on the website, their details (Name, Contact Number, Address, PIN Code, Password) will automatically appear here.'}
              </p>
            </div>
          ) : (
            <div className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-sm">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-100 text-[11px] font-extrabold uppercase text-slate-400 tracking-wider">
                      <th className="py-3.5 px-5">Customer Details</th>
                      <th className="py-3.5 px-5">Contact Number</th>
                      <th className="py-3.5 px-5">Address & PIN Code</th>
                      <th className="py-3.5 px-5">Security PIN / Pass</th>
                      <th className="py-3.5 px-5">Order History</th>
                      <th className="py-3.5 px-5 text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-700">
                    {filteredUsers.map((user) => {
                      const userOrders = orders.filter(o => o.details.customerPhone === user.contactNumber);
                      const totalSpent = userOrders.reduce((sum, o) => sum + o.total, 0);
                      const showPassword = Boolean(showPasswordMap[user.contactNumber]);

                      return (
                        <tr key={user.contactNumber} className="hover:bg-slate-50/80 transition-colors">
                          <td className="py-4 px-5">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100 inline-block mt-0.5">
                                  Registered Member
                                </span>
                              </div>
                            </div>
                          ...
                          <td className="py-4 px-5">
                            <a 
                              ...
                              className="font-bold text-slate-900 hover:text-amber-600 inline-flex items-center gap-1.5 transition-colors"
                            >
                          ...
                          <td className="py-4 px-5 max-w-xs">
                            <div className="space-y-0.5">
                              <span className="text-slate-700 block truncate">{user.address}</span>
                              <span className="text-[10px] font-bold text-slate-400 inline-flex items-center gap-1">
                                <MapPin className="h-3 w-3" /> PIN: {user.pinCode}
                              </span>
                            </div>
                          </td>
                          ...
                          <td className="py-4 px-5">
                            <div className="inline-flex items-center gap-2 bg-slate-100 px-2.5 py-1 rounded-lg border border-slate-200">
                              ...
                              <button
                                onClick={() => togglePasswordShow(user.contactNumber)}
                            ...
                            <div className="flex items-center justify-end gap-2">
                              {userOrders.length > 0 && (
                                <button
                                  ...
                                  className="px-2.5 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-800 text-[11px] font-bold rounded-lg transition-colors inline-flex items-center gap-1 border border-amber-200/60 cursor-pointer"
                                >
                              ...
                              <button
                                onClick={() => handleDeleteUser(user.contactNumber, user.fullName)}
          ...
          {selectedUserForOrders && (
            ...
            >
              <div 
                className="bg-white rounded-3xl max-w-2xl w-full p-6 space-y-5 border border-slate-100 shadow-2xl max-h-[85vh] overflow-y-auto"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                  <div>
                    <h3 className="text-lg font-black text-slate-900">
                      Order History: {selectedUserForOrders.fullName}
                    </h3>
                    <p className="text-xs text-slate-500">
                      Contact: {selectedUserForOrders.contactNumber} • PIN: {selectedUserForOrders.pinCode}
                    </p>
                  </div>
                  <button 
                    onClick={() => setSelectedUserForOrders(null)}
                ...
                <div className="space-y-3">
                  {orders
                    .filter(o => o.details.customerPhone === selectedUserForOrders.contactNumber)
                    .map((order) => (
                      <div key={order.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-200/80 space-y-2">
                        ...
                        <div className="text-xs text-slate-600 space-y-1">
                          {order.items.map((cartItem, idx) => (
                            <div key={idx} className="flex justify-between">
                              ...
                              <span className="font-semibold text-slate-800">₹{cartItem.item.price * cartItem.quantity}</span>
                            </div>
                          ))}
                        </div>

                        <div className="pt-2 border-t border-slate-200 flex justify-between items-center text-xs font-black text-slate-900">
                          <span className="text-slate-400 font-normal">{order.timestamp}</span>
