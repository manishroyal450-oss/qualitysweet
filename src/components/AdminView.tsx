...
import { MenuItem, Order, CartItem, ViewMode } from '../types';
import { 
  Lock, 
  Unlock, 
  Trash2, 
...
export default function AdminView({
  ...
  // UI state
  const [activeTab, setActiveTab] = useState<'orders' | 'inventory' | 'capacity' | 'standards'>('orders');
    ...
    <div className="space-y-8 pb-24" id="admin-workspace-container">
      ...
      <div className="flex border-b border-slate-200 gap-6">
        <button
          ...
          className={`pb-3 text-sm font-bold border-b-2 transition uppercase tracking-wider ${activeTab === 'orders' ? 'border-amber-600 text-amber-700' : 'border-transparent text-slate-500 hover:text-slate-900'}`}
        >
          Orders & Transactions ({orders.length})
        </button>
        <button
          onClick={() => setActiveTab('inventory')}
          className={`pb-3 text-sm font-bold border-b-2 transition uppercase tracking-wider ${activeTab === 'inventory' ? 'border-amber-600 text-amber-700' : 'border-transparent text-slate-500 hover:text-slate-900'}`}
        >
          Menu Inventory & Slots
        </button>
        <button
          onClick={() => setActiveTab('capacity')}
          className={`pb-3 text-sm font-bold border-b-2 transition uppercase tracking-wider ${activeTab === 'capacity' ? 'border-amber-600 text-amber-700' : 'border-transparent text-slate-500 hover:text-slate-900'}`}
        >
          Simulation Tools
        </button>
        <button
          onClick={() => setActiveTab('standards')}
          className={`pb-3 text-sm font-bold border-b-2 transition uppercase tracking-wider ${activeTab === 'standards' ? 'border-amber-600 text-amber-700' : 'border-transparent text-slate-500 hover:text-slate-900'}`}
        >
